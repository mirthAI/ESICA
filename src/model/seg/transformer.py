...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = Attention(embedding_dim, num_heads, num_kv_heads)
...
class Attention(nn.Module):
    def __init__(
        ...
        kv_total_dim = head_dim * self.num_kv_heads

        self.q_proj = nn.Linear(embedding_dim, self.internal_dim)
        self.k_proj = nn.Linear(embedding_dim, kv_total_dim)
        self.v_proj = nn.Linear(embedding_dim, kv_total_dim)

        self.out_proj = nn.Linear(self.internal_dim, embedding_dim)

    def _separate_heads(self, x: Tensor, n_heads: int) -> Tensor:
        b, n, c = x.shape
        x = x.reshape(b, n, n_heads, c // n_heads)
    ...
    def forward(
        ...
        q_heads = self._separate_heads(q, self.num_heads)
        k_heads = self._separate_heads(k, self.num_kv_heads)
        v_heads = self._separate_heads(v, self.num_kv_heads)
        ...
        if self.num_kv_heads < self.num_heads:
            ...
            k_heads = k_heads.repeat_interleave(num_repeats, dim=1)
            v_heads = v_heads.repeat_interleave(num_repeats, dim=1)

        _, _, _, c_per_head = q_heads.shape
        attn = q_heads @ k_heads.permute(0, 1, 3, 2)
