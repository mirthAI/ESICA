...
def DecomposedStem(inp, oup, image_size, kernel_size, downsample=False):
    return nn.Sequential(
        DecompConv3D(inp, oup, 13, 1, 1, nn.GELU(), fd=True),
...
class DecompConv3D(nn.Module):
    def __init__(
        ...
        self.c1 = nn.Sequential(
            ...
            RMSNorm3D(out_dim) if norm else nn.Identity(),
    ...
    def forward(self, x):
        if self.fd:
            x = self.c1(x) + self.c2(x) + self.c3(x) + self.c4(x)
        else:
...
class ChannelBlock(nn.Module):
    def __init__(self, dim, heads=8):
        ...
        self.layer_norm1 = nn.RMSNorm(dim)

        self.mlp1 = MLP(dim, hidden_dim)
        self.layer_norm2 = nn.RMSNorm(dim)
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.dwconv = DecompConv3D(oup, oup, kernel_size, groups=oup, fd=fd)
...
class DecompModel(nn.Module):
    ...
    def forward(self, video, mask=None, device="cuda"):
        ...
        #     hidden_states[i] = rearrange(hidden_states[i], "b c d h w -> b d h w c")
